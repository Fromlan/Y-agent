...
function MessageItem({
    ...
    return (
      <div className="flex justify-end">
        <div className="max-w-[70%]">
          ...
          <div className="bg-accent/12 text-text-primary px-3.5 py-2.5 rounded-2xl rounded-tr-md text-sm whitespace-pre-wrap break-words shadow-sm border border-accent/20">
            {message.content}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex gap-2">
      ...
      <div className="max-w-[80%] flex-1 min-w-0 anim-fade-up">
...
function DemoPromptCard({
  ...
  return (
    <button
      ...
      className={`panel p-3 text-left hover:border-accent/40 hover:bg-bg-elev/60 transition-all duration-150 group anim-hover-pop ${
        accent ? "border-accent/30 bg-accent/5 shadow-sm" : ""
      }`}
    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Zap className={`w-3 h-3 flex-shrink-0 ${isRunning ? "text-accent anim-pulse-soft" : "text-accent"}`} />
        ...
        {isRunning && (
          <span className="text-text-muted text-[10px] flex items-center gap-1">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-accent anim-pulse-soft" />
            运行中
          </span>
        )}
