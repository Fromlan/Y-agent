...
- `doc/` — 项目文档（plan.md / dev.md / api-integration-notes.md / 即梦api.md / 即梦提示词参考.md）
- `public/` — 静态资源
...
- **没有 e2e 测试**。每个里程碑靠手动冒烟：见 `doc/plan.md` 各 M 节点
...
- 新功能必须：
  ...
  5. 如果动了 API 客户端 / Skill 模板，**新增/修改要更新 `doc/api-integration-notes.md`**

## PR & commit conventions

- Branch from `main`（当前默认分支）；不要直接 push 到 main
- Commit message：conventional commits（`feat:` / `fix:` / `docs:` / `refactor:` / `chore:`）
- 一个 commit 只做一件事。Skill 模板新增 = 一个 commit；API 客户端重构 = 一个 commit
- CI（`.github/workflows/ci.yml`）跑 `pnpm lint` + `pnpm test` + `pnpm build` + `cargo check`，所有必须绿
- 发版前在 SettingsPanel「关于」行确认 `vX.Y.Z · (short-sha build-date)` 显示正确
