#!/usr/bin/env python3
"""Add video branch to AssetDetailDialog.tsx — v3.

Strategy: find the `// 模式与 tab 同步：` comment (which appears in a known
location near the top of the function), and insert the video early-return
block right after it. This avoids the rfind ambiguity.
"""
...
def main() -> int:
    ...
    # Insert the video early return block right before the FIRST `return (` of the main component.
    # We use a distinctive marker: the line just before the main `return (` has a specific
    # pattern. The main component has `  const onUp = () => setDrag(null);` followed by
    # `  return (`. But there's a conditional return at line 1127 too.
    # The cleanest approach: find the marker `  const onUp = () => setDrag(null);` which
    # is unique to this component, and insert the video block right after it.
